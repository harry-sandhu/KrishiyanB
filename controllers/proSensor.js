...
exports.getSensorByContactAndField = async (req, res) => {
  const { contactNumber, fieldId } = req.params;

  try {
    // Query the database to find all sensor data based on contact number and field ID
    const sensorData = await Sensor.find({ contactNumber, fieldId });

    if (sensorData.length === 0) {
      return res.status(404).json({ message: "Sensor data not found" });
    }

    // Return the found sensor data
    res.json(sensorData);
  } catch (error) {
